...
To structure the treatment, the backgrounds are categorised into three main classes:
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.55\textwidth]{Figures/Chapter6/FakeFactorSchematic.pdf}
%     \caption[Schematic of control and application regions in the Fake Factor method.]{Illustration of the typical control region topology used in the classical Fake Factor method. Corrections are applied to bridge kinematic mismatches between regions.}
%     \label{Figure:FakeFactorSchematic}
% \end{figure}

These limitations motivate the development of more flexible reweighting strategies that can incorporate multiple variables simultaneously without the constraints of explicit binning.
...
\subsubsection{BDT-Based Reweighting}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.75\textwidth]{Figures/Chapter6/BDT_Classifier_vs_Reweighter.pdf}
%     \caption[Comparison between standard BDT classifier and $\chi^2$-based BDT reweighter.]{Illustration of the difference between standard BDT classification loss (left), which focuses on decision boundaries, and the $\chi^2$-based BDT reweighter (right), which targets regions with the largest MC--data discrepancies.}
%     \label{Figure:BDTvsClassifier}
% \end{figure}
