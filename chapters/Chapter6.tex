...
The \textbf{\ac{DY}} process, $\PZ/\gamma^* \to \ell^+\ell^-$, is one of the dominant backgrounds in ditau final states. Among the leptonic DY decays ($\PZ/\gamma^* \to e^+e^-$, $\mu^+\mu^-$, and $\tau^+\tau^-$), the $\tau^+\tau^-$ channel accounts for roughly one third of events, providing a source of genuine $\tau$ leptons. In addition, QCD radiation can produce accompanying jets in the final state, as illustrated in Fig.~\ref{Figure:Chapter6_DY}. Although the overall DY contribution is suppressed in this analysis, it remains relevant through three distinct mechanisms:
...
\begin{figure}[!htbp]
    ...
    \caption[Examples of Feynman diagrams for Drell-Yan without additional partons and one parton originating from initial state radiation.]{Examples of Feynman diagrams for \ac{DY} \textbf{(a)} without additional partons and \textbf{(b)} one parton originating from \ac{ISR}.}
...
{Summary of the simulated \ac{SM} backgrounds used in this search, including their generators and the perturbative order (\ac{LO}, \ac{NLO}, or \ac{NNLO}) of cross sections.}
...
+ jets, $10 < m_{\ell \ell} < 50\GeV$ & \MADGRAPH, \PYTHIA & 15810.0 (\ac{LO}), 18610.0 (\ac{NLO}\hyperlink{Stat_precision}{$^1$}) \\
+ jets, $m_{\ell \ell} > 50\GeV$ & \MADGRAPH, \PYTHIA & 5379.0 (\ac{LO}), 6077.2 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$}) \\
...
+ jets & \MADGRAPH, \PYTHIA & 52940.0 (\ac{LO}), 61526.7 (\ac{NLO}\hyperlink{Stat_precision}{$^1$}) \\
...
\rowcolor{verylightblue}
\textbf{\ttbar (\ac{NLO})} & & \\
Fully hadronic & \POWHEG, \PYTHIA & 377.96 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$})\\
Semi-leptonic & \POWHEG, \PYTHIA & 365.34 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$})\\
Fully leptonic & \POWHEG, \PYTHIA & 88.29 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$}) \\

\arrayrulecolor{lightgray}\hline
\rowcolor{verylightblue}
\textbf{Single top (\ac{NLO})} & & \\
t-channel ($t$) & \POWHEG, \PYTHIA & 136.02 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$}) \\
t-channel ($\overline{t}$) & \POWHEG, \PYTHIA & 80.95 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$}) \\
$t + W^-$ & \POWHEG, \PYTHIA & 35.60 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$}) \\
$t + W^+$ & \POWHEG, \PYTHIA & 35.60 (\ac{NNLO}\hyperlink{Stat_precision}{$^1$}) \\

\arrayrulecolor{lightgray}\hline
\rowcolor{verylightblue}
\textbf{Diboson (\ac{LO}/\ac{NLO})} & & \\
...
$gg \rightarrow ZZ \rightarrow e \, e \, \PGt \, \PGt $ & \PYTHIA & $3.194e^{-3}$ (\ac{LO})\\
$gg \rightarrow ZZ \rightarrow \mu \, \mu \, \PGt \, \PGt $ & \PYTHIA & $3.194e^{-3}$ (\ac{LO})\\
$gg \rightarrow ZZ \rightarrow \mu \, \mu \, \mu \, \mu $ & \PYTHIA & $1.585e^{-3}$ (\ac{LO})\\
$gg \rightarrow ZZ \rightarrow \PGt \, \PGt \, \PGt \, \PGt $ & \PYTHIA & $1.585e^{-3}$ (\ac{LO})\\
...
\footnotesize
\hypertarget{Stat_precision}{}$^{1}$While the samples are generated at the perturbative order indicated by the generator, the normalisation is performed using the best available cross sections. For example, the inclusive \ac{DY}+jets sample with $m_{\ell\ell} > 50\GeV$ is generated at \ac{LO}, but normalised to the \ac{NNLO} prediction. \\
...
The signal samples are simulated under the \textit{alignment limit} of the \ac{2HDM}~\cite{TypeX_2HDM}, consistent with current experimental constraints on the couplings of the observed Higgs boson. The specific scenario is dynamically selected at each mass point depending on the value of $m_\phi$:
\begin{enumerate}[label=(\roman*)]
...
For each signal point, the distributions correspond to the visible mass\footnote{The visible mass is defined as the invariant mass of the visible decay products of the two $\PGt$ leptons.} of the $\tau^+\tau^-$ pair originating from a single scalar decay ($\phi \to \tau^+\tau^-$ or $A \to \tau^+\tau^-$). Since the process $Z^* \to \phi A$ yields two Higgs bosons per event, and hence two independent $\tau$ pairs, the visible mass is computed separately for each decay, and the resulting distributions are shown for the correctly matched generator-level pairs in Fig.~\ref{Figure:Chapter6_GenVisDistributions}.
...
\begin{figure}[!htbp]
        ...
        \begin{subfigure}[b]{0.7\textwidth}
            ...
            \caption{}
        \end{subfigure}
    \caption[Generator-level visible mass distributions for different mass combinations of $\phi$ and A.]{
    Generator-level distributions of the visible di-$\PGt$ mass for selected combinations of $m_A$ and $m_\phi$. 
    The visible mass is computed using only the decay products of the two $\PGt$ leptons, excluding neutrinos. 
    Distributions are shown for scans over \textbf{(a)} $m_A$ and \textbf{(b)} $m_\phi$. 
    It is worth noting that the visible mass distributions for $A$ and $\phi$ exhibit a mild dependence on the mass of the other scalar. 
    This arises from the kinematic correlation inherent in the two-body production process $Z^* \to \phi A$: increasing $m_\phi$ reduces the available phase space for $A$, thereby decreasing its boost and shifting $m_A^{\text{vis}}$ to lower values (and vice versa). 
    This effect is more pronounced in panel~(a), where for each value of $m_A$ the corresponding $m_\phi$ points span a wider mass range (\eg 100–300\GeV). 
    In contrast, panel~(b) shows a weaker variation because, for each $m_\phi$, the associated $m_A$ values cover a narrower interval (\eg 60–160\GeV).
    }
...
\arrayrulecolor{lightgray} \hline
Double-Tau ($\PGt \PGt$)         & 35     & 35     & 35     & 35     & 35     & 35     \\
\arrayrulecolor{black} \hline
\end{tabular}
...
$e_1 \mathbin{\lor} \mu_2 \mathbin{\lor} \tau_3\tau_4$ \\
\arrayrulecolor{lightgray} \hline
...
Offline selection criteria are applied to reconstructed electrons, muons, and $\PGt_h$ candidates, as introduced in Chapter~\ref{Section:Chapter4}. These selections suppress backgrounds from nonprompt and misidentified particles while maintaining high signal efficiency. To ensure consistency with the \ac{PV} and reduce contamination from \ac{PU}, all selected objects are required to originate from the \ac{PV}. This is enforced through cuts on transverse and longitudinal \acp{IP}: $|d_{xy}| < 0.045\unit{cm}$ and $|d_z| < 0.2\unit{cm}$ for electrons and muons, and $|d_z| < 0.2\unit{cm}$ for $\PGt_h$ candidates\footnote{The \ac{IP} selections are designed to enforce compatibility with the primary vertex, exploiting the fact that prompt electrons, muons, and the charged prongs of hadronic $\tau$ decays originate within $\mathcal{O}(100~\mu\mathrm{m})$ of the PV. The typical track impact-parameter resolution in CMS is at the level of $10$--$20~\mu\mathrm{m}$ in $d_{xy}$ and $20$--$40~\mu\mathrm{m}$ in $d_{z}$, so the requirements correspond to ``loose promptness'' selections. Although the $\tau$ lepton has a finite lifetime, its visible decay products are still reconstructed with \acp{IP} well within these thresholds. Nonprompt leptons and misidentified hadronic objects from heavy-flavour decays or pileup vertices typically fail these criteria, making the IP cuts effective for suppressing such backgrounds while retaining high signal efficiency.}.

Electrons and muons are additionally required to be isolated from nearby hadronic activity. The relative isolation, defined in Chapter~\ref{Section:Chapter4} (Equations~\ref{Equation:Chapter4_PFIso_Electron} and~\ref{Equation:Chapter4_PFIso_Muon}), is required to satisfy $I^{e/\mu}_\text{PF} < 0.15$. No explicit isolation cut is applied to $\PGt_h$ candidates, as this information is embedded in the DeepTau discriminators.
...
% Move the caption BEFORE the table
\begin{table}[!htbp]
\caption[Event-level selection requirements applied to each four-tau channel.]{
Event-level selection criteria applied to each four-tau channel. The table specifies the required number of electrons, muons, hadronic taus, and $b$-tagged jets, as well as the total charge requirement for the selected objects.}
...
\label{Section:Chapter6_Corrections}
...
\begin{enumerate}[label=(\roman*)]
    \item Identification: No requirement on identification, isolation, or trigger.
...
where $N_{\text{pass}}$ and $N_{\text{fail}}$ are the fitted $Z \rightarrow \ell^+\ell^-$ yields in each category. Figure~\ref{Figure:Chapter6-TagAndProbeFits_Nominal} shows an example fit for electrons in a representative $\eta$ bin.
...
Efficiency \acp{SF} are then computed as the ratio of data and simulation efficiencies:

\begin{equation_pad}
    \text{SF}(p_\text{T},\eta) = \frac{\epsilon_\text{data}(p_\text{T},\eta)}{\epsilon_\text{MC}(p_\text{T},\eta)}
\end{equation_pad}

where these \acp{SF} are applied per object, and the total event-level correction is given by the product of the \acp{SF} for all leptons in the event.

Systematic uncertainties are estimated by varying aspects of the tag-and-probe procedure. These variations include modifying the tag lepton $p_{\text{T}}$ threshold (\eg from $25\GeV$ to $35\GeV$), using alternative signal models, and changing the background parameterisation. Figure~\ref{Figure:Chapter6-TagAndProbeFits_Alternative} shows a representative comparison between the nominal fit configuration and a variant with a tighter tag lepton requirement. The resulting differences in efficiency are propagated as systematic uncertainties on the \acp{SF}.
...
Reliable modelling of $\PGt_h$ leptons is essential for accurate signal and background estimation in this analysis. As with electrons and muons (see Sections~\ref{Section:Electron_Identification} and~\ref{Section:Muon_Identification}), data-to-simulation corrections are applied in the form of \acp{SF}. These corrections are derived centrally by the \ac{CMS} collaboration~\cite{DeepTau_24-001}. These corrections are discussed in more detail in Section~\ref{Section:Chapter7_Tau_ID_ES}.

\subsubsection{Trigger}
\label{Section:Chapter6_HadronicTauTrigger_Efficiencies}
Trigger \acp{SF} for $\PGt_h$ candidates are also derived using $\PZ/\gamma^* \to \tau_\mu \tau_h$ events, exploiting the clean topology and well-identified muon leg. As with the identification and energy scale corrections, these are provided centrally by the \ac{CMS} collaboration and are therefore only summarised here.
...
\item \textbf{Jet $\to \PGt_h$ misidentification:} Backgrounds with one or more jets misidentified as $\PGt_h$ are modelled using a data-driven fake factor ($F_F$) method.

\item \textbf{Lepton $\to \PGt_h$ misidentification:} Although subdominant, backgrounds from electrons or muons misidentified as $\PGt_h$ can affect specific channels. These are estimated using simulation with dedicated corrections, as detailed in Section~\ref{Section:Chapter6_Lepton_MisID_SF}.

\end{enumerate}

\subsection{\texorpdfstring{Genuine-$\PGt$ backgrounds from ZZ}{Genuine tau backgrounds from ZZ}}
\label{Section:Chapter6_GenuineBackground}

These backgrounds arise predominantly from quark–antiquark annihilation, with a smaller contribution from gluon–gluon fusion. As discussed in Section~\ref{Section:Chapter6_Backgrounds}, the fully leptonic final state and close kinematic resemblance to the signal make this background particularly challenging to suppress. As listed in Table~\ref{Table:Chapter6_SimulatedBackgrounds}, the quark-initiated process is simulated using \POWHEG at \ac{NLO} in \ac{QCD}, while the gluon-initiated component is generated at \ac{LO} with \PYTHIA. To account for missing higher-order effects, both samples are corrected using multiplicative $\mathcal{K}$-factors~\cite{Kfactors_ZZ} applied as a function of the four-lepton invariant mass, $m_{4\ell}$:
...
This section discusses how the background from jets misidentified as $\PGt_h$ (abbreviated as Jet $\to \PGt_h$) is modelled. These backgrounds arise from misidentified jets from processes such as the ones outlined in Section~\ref{Section:Chapter6_Backgrounds}. Such misidentifications are particularly problematic to model in simulation, as the jet $\to\PGt_h$ misidentification rate is poorly described in \ac{MC} simulations. Moreover, the small probability of such a jet misidentification makes it necessary to generate high-statistics \ac{MC} samples, which come with a significant computational expense. These limitations and the difficulty in capturing the full range of misidentification scenarios in simulation motivate the development and use of a data-driven approach, known as the $F_F$ method.

\subsubsection{Classical \texorpdfstring{$F_F$}{Fake Factor} Method}
...
As noted in Section~\ref{Section:Chapter6_FakeFactors_Classical}, backgrounds from non-jet $\to\PGt_h$ candidates are traditionally removed by subtracting simulated events from data at the histogram level. However, this approach is not compatible with the \ac{ML}-based method, which operates on full unbinned datasets and does not support negative event weights. One possible approach is template matching, where a simulated non-jet $\to \PGt_h$ candidate is removed for every matching data event to emulate histogram subtraction at the event level. Yet, this method is computationally impractical given the high dimensionality of the input space and the size of the datasets.
...
\circnum{1} $F_F^1$ & 0 & 1 & 1 \\
\arrayrulecolor{lightgray} \hline

\circnum{2} $F_F^2$ & 1 & 0 & 1 \\
\arrayrulecolor{lightgray} \hline

\circnum{3} $F_F^1 \times F_F^2$ & 0 & 0 & 1 \\
\arrayrulecolor{black} \hline
\circnum{1} + \circnum{2} - \circnum{3} & 1 & 1 & 1 \\
\arrayrulecolor{black} \hline
\end{tabular}
...
\circnum{1} $F_F^1$ & 1 & 1 & 1 & 0 & 0 & 1 \\
\arrayrulecolor{lightgray} \hline

\circnum{2} $F_F^2$ & 0 & 1 & 1 & 1 & 1 & 0 \\
\arrayrulecolor{lightgray} \hline

\circnum{3} $F_F^3$ & 0 & 0 & 1 & 1 & 0 & 1 \\
\arrayrulecolor{lightgray} \hline

\circnum{4} $F_F^1 \times F_F^2$ & 0 & 1 & 1 & 0 & 0 & 0 \\
\arrayrulecolor{lightgray} \hline

\circnum{5} $F_F^1 \times F_F^3$ & 0 & 0 & 1 & 0 & 0 & 1 \\
\arrayrulecolor{lightgray} \hline

\circnum{6} $F_F^2 \times F_F^3$ & 0 & 0 & 1 & 1 & 0 & 0 \\
\arrayrulecolor{lightgray} \hline

\circnum{7} $F_F^1 \times F_F^2 \times F_F^3$ & 0 & 0 & 1 & 0 & 0 & 0 \\
\arrayrulecolor{black} \hline

\makecell[c]{%
  \circnum{1} + \circnum{2} + \circnum{3} \\ 
  - \circnum{4} - \circnum{5} - \circnum{6}  \\ 
   + \circnum{7}
}
& 1 & 1 & 1 & 1 & 1 & 1 \\
\arrayrulecolor{black} \hline
...
This section will discuss the statistical and systematic uncertainties that are taken into account in this analysis. The uncertainties related to the limited number of simulated events are taken into account with the Barlow-Beeston-lite approach~\cite{BARLOW1993219,Conway:2011in}, while the systematic uncertainties are grouped into two main categories:
...
\subsection{Electron and muon corrections}
...
A 2\% uncertainty is applied to account for residual variations in the corresponding \acp{SF}, due to modelling choices in the tag-and-probe procedure (\eg, tag selection, background and signal parameterisation). Though conceptually a normalisation uncertainty, it is implemented as a shape uncertainty to reflect the kinematic dependence of the \acp{SF} across $p_{\mathrm{T}}$ and $\eta$.
...
\subsection{Hadronic tau corrections}
\label{Section:Chapter6_UModel_HadronicTau}

Uncertainties associated with $\PGt_h$ identification and energy scale \acp{SF} include:
...
\label{Section:Chapter6_UModel_LeptonMisID}
...
Uncertainties related to the theoretical modelling of the signal include both shape and normalisation components. Their treatment depends on the statistical interpretation. In interpretations that set limits directly on the number of observed signal events in each category, only the kinematic distortions are retained as shape variations, while the overall yield shifts are removed. In interpretations where the expected signal yield is normalised to a theoretical cross section, both the shape variations and their corresponding yield effects are propagated. The typical impact of the latter (normalisation-only) components is:

\begin{itemize}
\item \textbf{\ac{PDF}}: Derived from the envelope of \ac{PDF} weight variations; $\sim$6\%.
...
Multiple sources of uncertainty are associated with the \ac{ML}-based $F_F$ method. These uncertainties are treated as uncorrelated across final states, with the exception of the $\tauh\tauh\tauh\tauh$ and $\tauh\tauh\tauh$ channels, where the same fit is used. The main sources of uncertainty are summarised below:

\begin{itemize}
    \item \textbf{Contamination from non-jet $\to\PGt_h$ backgrounds in the fit region:}
    \begin{itemize}
        \item \textit{\ac{BDT}-based background removal} (Section~\ref{Section:Chapter6_BDT_NonJet_BkgRemoval}): An uncertainty is assigned by comparing the variable distributions obtained using standard histogram subtraction and those obtained with the \ac{BDT}-based subtraction. The largest observed deviation in each event is taken as the systematic uncertainty. This comparison is performed separately in the pass and fail $\tauh$ identification regions. Uncertainties that range from 0--5\% depending on the channel, are included to account for the influence of non-jet$\to\tau_h$ objects in the measurement regions.
    ...
    \begin{itemize}
        \item \textit{Fit closure:} An uncertainty is derived by comparing the reweighted distributions in the fail $\tauh$ identification region to those in the pass region. Any observed non-closure is propagated as a systematic uncertainty. This has size $\mathcal{O}(10\%)$ on the total jet$\to \tau_h$ background in each channel.
        \item \textit{Sideband variable dependence:} An uncertainty is placed to cover any potential dependency of the $F_F$ on the choice of sideband variables. To quantify this, alternative sideband variable combinations are used in separate fits. The largest resulting deviation is taken as the uncertainty. This uncertainty is found to be around 5--15\%, depending on the channel.
    \end{itemize}
\end{itemize}
...
The extraction of the signal is performed via a simultaneous binned maximum‑likelihood fit\footnote{An unbinned likelihood fit is in principle statistically optimal, but in CMS analyses a binned approach is preferred because systematic uncertainties, background templates, and data-driven estimates are naturally implemented as binned shapes.} to the distribution of the final discriminating variable in all channels and categories. The likelihood function, denoted by $\mathscr{L}$, is defined as:
...
\label{Section:Chapter6_NuisanceParameters}
...
Nuisance parameters that influence only the overall normalisation of a process are modelled with log-normal \acp{P.D.F}. This choice ensures that the scaling factors applied to template yields remain strictly positive. The functional form of this \ac{P.D.F} is:
...
Nuisance parameters that modify the shape of a distribution are assigned a Gaussian \acp{P.D.F}. The functional form of this \ac{P.D.F} is:
...
Upper limits on the signal strength $\mu$ are set using the modified frequentist approach~\cite{Junk:1999kv,Read:2002hq}, known as the CL$_\mathrm{s}$ method. The test statistic is defined as
...
The probability of obtaining a test statistic value at least as large as that observed, assuming a signal‑plus‑background hypothesis with strength $\mu$, can be expressed as,
...
The 95\% confidence level (CL) upper limit on the signal strength $\mu$ is defined as the smallest value of $\mu$ for which 
$\mathrm{CL}_s(\mu) < \alpha$. In this analysis, $\alpha = 0.05$, corresponding to the 95\%~CL.
...
The rejection of jets misidentified as $\PGt_h$ candidates is critical for the sensitivity of the search. The optimal $D_{\text{jet}}$ \ac{WP} is determined from a dedicated study using simulated signal and background samples, with a binary classifier. All dominant background processes discussed in Section~\ref{Section:Chapter6_Backgrounds} are included from simulation, scaled to the full Run 2 integrated luminosity. A low-mass signal benchmark is chosen as the training benchmark to optimise rejection performance in the most challenging regime; consistency checks with higher-mass benchmarks confirmed that the selected working point also performs robustly across the full mass range.
...
\begin{enumerate}
    ...
    \item Events with one or more jets misidentified as $\tau_{h}$ leptons.
    \item Events in which light leptons are misidentified as $\tau_{h}$ leptons (no jet$\to\tau_\mathrm{h}$ misidentification).
\end{enumerate}
