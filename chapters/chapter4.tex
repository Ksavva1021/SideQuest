...
Analyses targeting final states with multiple $\PGt$ leptons (two or more) are affected by a variety of Standard Model (SM) processes that can produce genuine or misidentified $\PGt$ leptons. This section introduces the main simulated background processes used in such analyses, with a focus on final states containing two $\PGt$ leptons, which provides a clear and representative overview. Although the analysis involving four $\PGt$ leptons is also a major component of this thesis, it introduces additional complexities such as higher rates of object misidentification and a broader range of background contributions. These are discussed in detail in Section~\ref{Section:Chapter_4tau}. The simulated samples used across both analyses are largely the same, with the main exception being quark- or gluon-initiated production of two $\PZ$ bosons.

The dominant backgrounds in the two-$\PGt$ final state are Drell–Yan, $\ttbar$, and diboson processes. Additional contributions arise from $\PW$+jets and QCD multijet events, where jets are misidentified as hadronic $\PGt$ decays. While these account for most of the background, subdominant contributions include triboson production, single-top processes, and electroweak production of $\PW$ and $\PZ$ bosons.
