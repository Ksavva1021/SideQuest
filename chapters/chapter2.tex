...
where $c(\theta^+, \theta^-)$ is defined as $\sin(\theta^+)\sin(\theta^-)/(1+\cos(\theta^+)\cos(\theta))$. The angles $\theta^\pm$ and $\phi^\pm$ are the polar and azimuthal coordinates of the \ac{PV}, which is the optimal estimator of the $\tau$ polarisation. The \ac{PV} is taken with respect to the $\tau^\pm$ direction of flight, as illustrated in Fig~\ref{Figure:Chapter2_PolarimetricVector_Definition}.
