...
The tracking performance of the RAW' format appears promising, showing a small degradation in efficiency across both $\eta$ and $p_\mathrm{T}$, typically of the order of a few per cent. However, it is also important to ensure that this also holds for the efficiency as a function of the vertex radius, as this directly impacts the reconstruction of displaced tracks. As shown in Fig.~\ref{Figure:Chapter5_TrackingPerformance_vertexPos_1}, there is a notable decrease in performance beyond a radius of approximately $10\unit{cm}$, with the efficiency becoming severely degraded above $20\unit{cm}$. This behaviour is consistent with the corresponding inefficiencies observed at low~$\eta$ and high~$p_{\mathrm{T}}$. Tracks originating at large radial distances can only be reconstructed if they intersect the barrel layers of the tracker, which geometrically restricts them to low pseudorapidity; high-$|\eta|$ tracks escape through the endcaps before reaching such radii. In addition, only high-$p_{\mathrm{T}}$ tracks exhibit sufficiently small curvature in the magnetic field to avoid curling inwards before traversing the outer strip layers. Consequently, the displaced-track phase space at large vertex radii is naturally populated by low-$\eta$, high-$p_{\mathrm{T}}$ tracks, explaining why the inefficiencies in these kinematic regions mirror the drop in performance as a function of vertex radius.
