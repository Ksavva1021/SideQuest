...
However, not all corrections from Chapter~6 are directly transferable to this analysis. Certain ingredients must either be re-derived or adapted due to the specific selections employed here, in particular:
\begin{itemize}
    \item The use of ParticleNet for \ac{DM} classification instead of the \ac{HPS} algorithm 
    \item The optimised, \ac{DM}–dependent selections summarised in Table~\ref{Table:Chapter7_RefinedSelections_perDM}.
\end{itemize}

Corrections that differ from those used in the four-tau analysis will be discussed in more detail in the remainder of this section. 
...
\begin{equation}
    w = \sqrt{\text{SF}(p^t_\text{T}) \, \,\cdot \, \, \text{SF}(p^\overline{t}_\text{T})}
\end{equation}
...
\subsection{ggH NNLOPS reweighting}
