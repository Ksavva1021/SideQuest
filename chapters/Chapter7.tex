...
For \acp{DM} where the \ac{IP} method or the \ac{P.V.} method is used (for both $\tau$ leptons), the situation is different. Here, smearing effects in the reconstruction of the \ac{PV} are correlated between the two decay planes, leading to a depletion around $\phi_{\mathrm{CP}} = 180^\circ$ \cite{Berge:2014sra}. 
