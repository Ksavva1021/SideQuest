...
Building on the analysis strategy outlined in the previous sections, this section presents the post-fit distributions, along with the model-independent and model-dependent interpretations of the results. Further details are discussed in the corresponding subsections.
...
Postfit distributions of the $m_\mathrm{T}^\mathrm{tot}$ discriminator are shown after performing a simultaneous background-only fit to the data. The fit is carried out across all bins in all analysis categories. For visual clarity, the categories are grouped into three figures corresponding to low-, medium-, and high-statistics categories. This ordering facilitates both visual inspection and the discussion of statistical features in different event selections.  

The expected backgrounds are stacked and separated into the three groups defined in Section~\ref{Section:Chapter6_Background_Modelling}:  

\begin{enumerate}
    \item Events with genuine $\tau_{h}$ leptons.
...
A benchmark signal hypothesis ($m_A = 160\GeV$, $m_\phi = 200\GeV$) is also shown in the distributions, scaled to $0.01\unit{pb}$, approximately three times smaller than the predicted cross section for this process. Figure~\ref{Figure:Chapter6_PostfitDistributions} presents the results.  
...
In the $m_\phi = 100\GeV$ case, the limit is largely flat across $m_A$ and excludes values of $\tan\beta \gtrsim 2$.  
Below this, the branching fractions of the $h$ and $A$ bosons to $\tau$ leptons decrease sharply, with decays to $b\bar{b}$ becoming dominant as discussed in Section\ref{Section:Chapter6_production_xs_bf}.  For $m_\phi = 200\GeV$, in regions where $m_A$ is larger than $m_\phi-m_{\PZ}$ and $m_\phi$ is larger than $m_A-m_{\PZ}$, the exclusion limit does not depend strongly on $m_A$ and reaches a $\tan\beta$ value of approximately 1.5. However, in regions where the separation of $m_\phi$ and $m_A$ is greater than $m_{\PZ}$, the sensitivity is reduced, as the $\PH\rightarrow\PZ A$ or $A\rightarrow\PZ\PH$ decay channels become kinematically accessible and dominate the branching fraction.
...
\begin{figure}[!htbp]
  ...
  \caption{
   ...
   The allowed regions accommodating the current measurement of the muon anomalous magnetic moment, as detailed in Ref.~\cite{Jueid:2021avn}, are shown as green dashed boxes, with the $\tan\beta$ allowed range written on the plot.
  }
  \label{Figures:Chapter6_model_dependent_2d}
\end{figure}

In summary, a search has been performed for signatures of extended Higgs sectors in the four‑tau final state, targeting the process $Z^* \to \phi A \to \PGt^+\PGt^-\PGt^+\PGt^-$ in pp collisions at $\sqrt{s} = 13\TeV$ with the CMS detector, using the full Run 2 dataset corresponding to an integrated luminosity of $138~\mathrm{fb}^{-1}$. No significant excess above the SM expectation is observed, and the results are interpreted as upper limits on the production cross section times branching fraction, which range from $\mathcal{O}(0.4~\mathrm{fb})$ at high $(m_A, m_\phi)$ to $\mathcal{O}(200~\mathrm{fb})$ at low masses. These constraints exclude a substantial fraction of the parameter space of the Type‑X two‑Higgs‑doublet model, including the region compatible with an explanation of the muon anomalous magnetic moment as proposed in Ref.~\cite{TypeX_2HDM}.
