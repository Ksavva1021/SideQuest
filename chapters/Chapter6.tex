...
\subsection{Statistical uncertainties in simulated templates}

Statistical uncertainties from the finite size of the simulated signal and background samples are modelled using the Barlow–Beeston “lite” method~\cite{BARLOW1993219,Conway:2011in}. This introduces one uncorrelated nuisance parameter per bin and process, scaling the corresponding bin yield. In the BB-lite implementation, the maximum likelihood estimator for each nuisance parameter can be computed analytically, avoiding the need to float them in the main likelihood fit. This approach improves computational stability and reduces fit time while retaining the correct treatment of per‑bin statistical fluctuations.
